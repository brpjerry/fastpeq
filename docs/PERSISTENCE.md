...
| `.history/.deleted/<name>/<revision>.txt` | Native APO config text | User-deleted history revisions. Each file is moved here instead of removed, so it is preserved for manual recovery but ignored by history lists and version counts. |
